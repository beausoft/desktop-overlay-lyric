...
LRESULT OverlayLyric::HandleMessage(UINT message, WPARAM wParam, LPARAM lParam) {
	...
	{
		/// | WS_EX_TRANSPARENT
		SetWindowLong(hWnd, GWL_EXSTYLE, WS_EX_LAYERED | WS_EX_TOOLWINDOW | WS_EX_TOPMOST);  // 设置窗口扩展样式
	...
	case WM_MOUSEMOVE: // 鼠标移动
	{
		if (mIsMousePrees) {
			...
			if (Utils::IsMouseCoordinate(MOUSE_X, MOUSE_Y)) {
				int y = winRect.top + (MOUSE_Y - mMouseXY.y);
				int x = winRect.left + (MOUSE_X - mMouseXY.x);
				MoveWindow(hWnd, x, y, winRect.right - winRect.left, winRect.bottom - winRect.top, TRUE);	//移动窗口
			}
		}
	}
	break;
